...
log.resultsFileName = log.addFolder(
    "simdata.dat"
)  # name of the dataset name (after processing the raw simulation data)
...
kuka.lowerLimits = [-0.967, -2, -2.96, 0.19, -2.96, -2.09, -3.05]  # for null space IK
kuka.upperLimits = [0.967, 2, 2.96, 2.29, 2.96, 2.09, 3.05]  # for null space IK
...
surftorch.As = torch.tensor([0, 0, 1.0])
surftorch.bs = torch.tensor(surftorch.pos[2] + 5e-3)
surftorch.fun = lambda r: binner(surftorch.As.to(r.device), r) - surftorch.bs.to(
    r.device
)  # constraint equation c(x)>=0
surftorch.fun_J = lambda r: surftorch.As.to(r.device).repeat(r.shape[0], 1)  # constraint Jacobian Jc(x)>=0
surftorch.fun_H = lambda r: torch.zeros((1, 3, 3), device=r.device).repeat(
    r.shape[0], 1, 1
)  # constraint Hessian  Hc(x)>=0


""" ---- config for the contact dynamics ----"""
contact_dynamics = Dict()
contact_dynamics.baumgarte_wn = 10.0
contact_dynamics.baumgarte_ksi = 1.0
contact_dynamics.friction.viscous.mu_C = 1.0  # viscous friction coefficient
...
trajplan.xy_Box = np.array(
    [  # x-y area where the end-effector will
...
sim = Dict()
sim.mode = [1, 2][1]  # 1=GUI, 2=without GUI
